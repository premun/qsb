			...
			if(rand(0,99) < $sance && 		# NEMYSLIS, ZAPLATIS!
			   ...
			   $hrac->posledni_bourani != $hrac->usek) {
				...
				if($hrac->kluzak->ma_pancerovani) {
					...
					$zavod->info->udalost($hrac->usek, $hrac->id, 'crash', $predmet, $dmg_kluzak, $dmg_pancir, round(100-100*min(abs(1-$dmg/$hrac->rychlost), 0.9)));
					
				} else {
					$hrac->kluzak->poskod($predmet,$dmg);
					$zavod->info->udalost($hrac->usek, $hrac->id, 'crash', $predmet, $dmg, 0, round(100-100*min(abs(1-$dmg/$hrac->rychlost), 0.9)));
				}					
						...
						if(rand(0,99) < $sance && 		
						   $utocnik->rychlost > $utocnik->reflexy/2) {
							...
							if($utocnik->kluzak->ma_pancerovani) {
								...
								$zavod->info->udalost($utocnik->usek, $utocnik->id, 'uhyb', $obet->id, $predmet, $dmg_kluzak, $dmg_pancir, round(100-100*min(abs(1-$dmg/$utocnik->rychlost), 0.9)));
								
							} else {
								$utocnik->kluzak->poskod($predmet,$dmg);
								$zavod->info->udalost($utocnik->usek, $utocnik->id, 'uhyb', $obet->id, $predmet, $dmg, 0, round(100-100*min(abs(1-$dmg/$utocnik->rychlost), 0.9)));
							}					
